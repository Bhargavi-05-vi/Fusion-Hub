...
import {
  FiStar, FiMapPin, FiCheck, FiWifi, FiMonitor, FiMusic,
  FiDroplet, FiUsers, FiCamera, FiGift, FiList, FiInfo,
...
const SESSIONS = [
  { id: "breakfast", label: "Breakfast", emoji: "🌅", startH: 8,  endH: 10, hideAfterH: 12 },
  { id: "lunch",     label: "Lunch",     emoji: "☀️", startH: 12, endH: 15, hideAfterH: 16 },
  { id: "dinner",    label: "Dinner",    emoji: "🌙", startH: 18, endH: 23, hideAfterH: 25 },
];
...
const OFFERS = [
  { title: "10% DineCash", desc: "Get flat 10% DineCash on your bill payment", badge: "🏷" },
  { title: "Free Dessert",  desc: "Complimentary dessert for bookings of 4+",  badge: "🎁" },
  { title: "Happy Hours",   desc: "20% off on drinks between 6–8 PM",           badge: "🍹" },
];
...
const TableReservationPage = () => {
  ...
  // ── Fetch restaurant from API (not mockData) ───────────
  const [restaurant, setRestaurant]         = useState(null);
  const [restaurantLoading, setRLoading]    = useState(true);
  ...
  const handleConfirm = async () => {
    ...
    try {
      await API.post("/reservations", {
        ...
        reservationDate: `${selectedDate}T${slot}`,
        guests,
        customerName:    name,
        phone,
      });
  ...
  const reset = () => {
    setConfirmed(false); setSlot(""); setName(""); setPhone(""); setErrors({});
  };
  ...
  if (restaurantLoading) {
    return (
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#000" }}>
        <div style={{ width:40, height:40, border:"2px solid rgba(249,115,22,0.3)", borderTopColor:"#f97316", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
    ...
    <div style={{ minHeight:"100vh", background:"#000", fontFamily:"'DM Sans', sans-serif", paddingTop:64 }}>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .card{background:#141414;border-radius:16px;overflow:hidden}
        .card-pad{padding:20px}
        .tabs{display:flex;border-bottom:1px solid #222}
        .tab-btn{flex:1;padding:14px 8px;font-size:13px;font-weight:600;color:#666;background:none;border:none;cursor:pointer;border-bottom:2px solid transparent;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:6px;font-family:'DM Sans',sans-serif}
        .tab-btn.active{color:#e23744;border-bottom-color:#e23744}
        .tab-btn:hover:not(.active){color:#999}
        .offer-card{display:flex;align-items:flex-start;gap:12px;padding:14px;border-radius:12px;margin-bottom:10px;background:#18181b;border:1px solid #27272a}
        .offer-badge{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;background:rgba(226,55,68,0.15)}
        .menu-section-header{display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-bottom:1px solid #2a2a2a;cursor:pointer}
        ...
        .amenity-item{display:flex;flex-direction:column;align-items:center;gap:7px;padding:14px 8px;border-radius:12px;background:#1a1a1a;border:1px solid #2a2a2a}
        .amenity-icon{width:36px;height:36px;border-radius:10px;background:rgba(226,55,68,0.1);display:flex;align-items:center;justify-content:center}
        .amenity-label{font-size:11px;font-weight:500;color:#888;text-align:center}
        ...
        .proceed-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 6px 24px rgba(226,55,68,0.4)}
      ...
      <div className="hero">
        ...
        <div className="hero-info" style={{ maxWidth:1160, margin:"0 auto", left:0, right:0, position:"absolute" }}>
          ...
          <div className="r-meta">
            ...
            <span className="r-meta-item"><FiMapPin size={13} /> {cityLabel}</span>
          </div>
        </div>
      </div>
      ...
      <div className="page-layout">
        ...
        <div className="left-panel">
            ...
            <div className="card-pad">
              ...
              {activeTab === "offers" && OFFERS.map((o) => (
                <div key={o.title} className="offer-card">
                  ...
                  <div>
                    <div style={{ color:"#fff", fontWeight:600, fontSize:14, marginBottom:4 }}>{o.title}</div>
                    <div style={{ color:"rgba(255,255,255,0.7)", fontSize:12 }}>{o.desc}</div>
                  </div>
                </div>
              ))}
              ...
              {/* MENU */}
              {activeTab === "menu" && (
                <div style={{ background:"#141414", borderRadius:12, padding:10 }}>
                  {MENU_SECTIONS.map((sec) => (
                    <div key={sec.name}>
                      <div className="menu-section-header" onClick={() => setExpandedMenu(expandedMenu === sec.name ? null : sec.name)}>
                        <span style={{ color:"#fff", fontSize:14, fontWeight:600 }}>
                          {sec.name} <span style={{ color:"rgba(255,255,255,0.4)", fontWeight:400 }}>({sec.items.length})</span>
                        </span>
                        {expandedMenu === sec.name ? <FiChevronUp size={16} color="#fff" /> : <FiChevronDown size={16} color="#fff" />}
                      </div>
                      {expandedMenu === sec.name && sec.items.map((item) => (
                        <div key={item.name} className="menu-item">
                          <div className="menu-item-left">
                            <div className="veg-dot" style={{ borderColor:item.veg?"#16a34a":"#dc2626", background:item.veg?"#16a34a":"#dc2626" }} />
                            <span style={{ color:"#fff", fontSize:13, fontWeight:500 }}>{item.name}</span>
                          </div>
                          <span style={{ color:"#fff", fontSize:13, fontWeight:600 }}>₹{item.price}</span>
                        </div>
                      ))}
                    </div>
                  ))}
          ...
          <div className="card card-pad">
            <div style={{ fontFamily:"'Sora',sans-serif", fontSize:"1rem", fontWeight:700, color:"#fff", marginBottom:8 }}>
              About {restaurant.name}
            </div>
            <p style={{ fontSize:14, color:"rgba(255,255,255,0.75)", lineHeight:1.8 }}>
              Experience premium dining at <strong style={{ color:"#fff" }}>{restaurant.name}</strong> — a celebrated destination for {cuisineLabel.toLowerCase()} cuisine in {cityLabel}.
          ...
          <div className="book-card">
            {confirmed ? (
              <div className="suc-wrap">
                ...
                <div style={{ fontSize:12, color:"rgba(255,255,255,0.35)", marginBottom:4 }}>Confirmed at {restaurant.name}</div>
                <div style={{ fontSize:11, color:"#22c55e", marginBottom:12 }}>✅ Saved to your reservation history</div>
              ...
              <>
                ...
                <div className="date-section">
                  ...
                  <div className="date-scroll">
                    {DAYS.map((d) => {
                      ...
                      return (
                        <div key={d.date} className={`date-chip${isSel?" sel":""}`}
                  ...
                  <div className="session-section">
                    ...
                    <div className="session-tabs">
                      {visibleSessions.map((s) => (
                        <button key={s.id} className={`sess-tab${resolvedSession===s.id?" sel":""}`}
                  ...
                  {visibleSessions.length === 0 ? (
                    ...
                    <>
                      <div className="slot-grid">
                        {displaySlots.map((s) => (
                          <button key={s.val} className={`slot-btn${slot===s.val?" sel":""}`}
                      ...
                      {allSlots.length > 12 && (
                        <button className="view-all-btn"
                          ...
                          {showAllSlots[resolvedSession]
                            ? <><FiChevronUp size={13}/> View less</>
                            : <><FiChevronDown size={13}/> View all slots</>}
                        </button>
                      )}
                    </>
                  )}
                ...
                <div className="form-section">
                  ...
                  <input type="text" className={`finput${errors.name?" err":""}`} placeholder="Full name"
                  ...
                  <input type="tel" className={`finput${errors.phone?" err":""}`} placeholder="10-digit mobile number"
                    value={phone} maxLength={10} onChange={(e) => { setPhone(e.target.value.replace(/\D/g,"")); setErrors((er) => ({ ...er, phone:undefined })); }} />
                ...
                <button className="proceed-btn" onClick={handleConfirm} disabled={bookingLoading}>
                  {bookingLoading ? "Booking..." : "Proceed"}
                </button>
