...
const seats = { VIP: { price: 0, label: 'VIP', color: 'from-yellow-500 to-orange-500', count: 20 }, Premium: { price: -500, label: 'Premium', color: 'from-purple-600 to-pink-500', count: 50 }, General: { price: -1000, label: 'General', color: 'from-blue-500 to-cyan-500', count: 100 } };
  ...
  if (booked) return (
    <div className="min-h-screen flex items-center justify-center pt-16 px-4">
      <div className="text-center max-w-sm">
        ...
        <p className="text-white/50 mb-1">Booking ID: #EV{Math.floor(Math.random() * 90000 + 10000)}</p>
        <p className="text-white/40 text-sm mb-2">{event.name}</p>
          ...
          <div className="bg-[#1A1A1A] rounded-2xl border border-white/5 p-6 h-fit sticky top-24">
            ...
            <div className="mb-5">
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => setQty(Math.max(1, qty - 1))} className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 text-white hover:border-purple-500/30 text-lg transition-all">−</button>
                <span className="text-white font-bold text-xl w-8 text-center">{qty}</span>
                <button onClick={() => setQty(Math.min(10, qty + 1))} className="w-9 h-9 rounded-xl bg-white/5 border border-white/10 text-white hover:border-purple-500/30 text-lg transition-all">+</button>
              </div>
            </div>
